...
基于 Three.js 与 Vite 构建的可交互暖通数字孪生展示。默认首屏直接进入 MAU-01 电影化剖视：透明外壳内显示正交安装的过滤器与盘管、直驱 EC 插入式离心风机、蒸汽加湿和消声段，红/蓝/青粒子表现空气状态变化，冷冻水支路与阀组显示内部水流。完整机房、制冷原理和通用设备透视仍可一键切换。
...
- G4/F8 过滤器、表冷盘管和再热盘管均按正交结构安装；送风段采用带喇叭形进风锥、七片后弯深筒叶片、后轮盘、直驱 EC 电机、支撑框架和减振件的插入式离心风机。
- MAU 气流在风机段沿轴向收敛进入叶轮眼，经后弯叶片径向排入静压箱，再恢复为正向送风；动画不再直接水平穿过叶轮侧面。
